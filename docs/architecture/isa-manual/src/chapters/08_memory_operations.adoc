...
[[memory-model]]
=== Memory model and ordering (v0.2)
...
Unless a platform profile tightens the rules, the v0.2 contract is:
...
Linx platforms classify memory addresses into types. This v0.2 manual uses two
high-level types:
...
The v0.2 Linx toolchain and emulator model are **little-endian**: the least-significant byte of a multi-byte value is
...
The v0.2 catalog includes load-reserved/store-conditional (`LR.*`/`SC.*`) and read-modify-write style atomic operations
with qualifier suffixes (e.g. `.aq`, `.rl`). The precise memory ordering semantics are defined by the ISA draft
(`isa/golden/v0.2/`) and the architectural model implemented by the toolchain/QEMU/RTL.
...
The v0.2 catalog defines two fence instructions:
